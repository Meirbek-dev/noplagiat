...
└───────────────────────────────────────────────────────────────────┘
 ...
 Browser / portal iframe                    · embed «/embed»
          │
          └──────────── Local sign-in (POST /api/auth/login, cookie session)
```
...
users(id PK, username TEXT UNIQUE (lower), password_hash TEXT NULL, email, display_name, active,
      sso_subject TEXT NULL UNIQUE)  -- SSO-era column, kept for the contract phase (ADR-017 §5)
...
- Конфигурация только через переменные окружения (`APP_DATABASE_URL`, `APP_LISTEN_ADDR`, `APP_PUBLIC_BASE_URL`, …), разбирается при старте в типизированный `Config`; при отсутствующих значениях - немедленное падение. `APP_PUBLIC_BASE_URL` - это внешне видимый origin шлюза, а никогда не внутренний адрес сервиса Rust или Node.
...
> Полный справочник по входу - [AUTH.md](AUTH.md): CLI учётных записей, форма сеанса и CSRF, роли и области видимости, гарантии и их гейты, диагностика.
...
├─ /api/internal/*  ── TraceLayer → SessionAuth → RbacScope extractor
│                      → AuditLayer (writes audit_log on response)
├─ /api/admin/*     ── same as internal + RequireRole(admin)
└─ /api/auth/*      ── POST login (rate-limited) / POST logout / GET me
```

- **`SessionAuth`**: проверяет сессионную cookie (`HttpOnly`, `Secure`, `SameSite=Lax`), загружает `users` + `user_roles` в расширение `CurrentUser`. Личность и гранты читаются из базы на **каждом** запросе, поэтому отзыв роли действует со следующего запроса, а не со следующего входа. Учётная запись без грантов → аутентифицирована, но не видит ничего внутреннего. Учётные записи создаёт CLI `manage-users`, ни один HTTP-маршрут (ADR-017).
...
| auth     | `POST /login` (логин + пароль, собственный ограничитель частоты), `POST /logout`, `GET /me`                                                                                                                                                                                                                                    |
...
├─ login.tsx                 # sign-in form: login name + password (ADR-017)
...
- Проверки в `beforeLoad` перенаправляют неаутентифицированных пользователей на `/login`, а пользователей без роли - на страницу запроса доступа. Клиентские проверки существуют только ради UX - авторизацию обеспечивает сервер (§4.2); e2e-тесты проверяют и то и другое.
...
| Атаки на сессии                                                  | пароли Argon2id; ротация идентификатора сессии при входе; ограничитель частоты на `POST /api/auth/login`; cookie с `HttpOnly/Secure/SameSite`; CSRF-токен на изменяющих маршрутах; строгий CSP; никаких токенов в URL                                                                                                                                                                             |
...
- **ADR-014** (§1–§3 замещены ADR-017) Внутренний контур отдаёт точные числа пяти ролям с областями видимости и подвергает скринингу всё, что покидает систему в виде файла; матрица RBAC - это реестр, с которым сверяется контракт.
...
- **ADR-017** Аутентификация - локальные учётные записи: логин и пароль (Argon2id) в таблице `users`, создаваемые CLI `manage-users` на хосте развёртывания. Поставщик идентичности, `dev-login` и сопоставление групп AD удалены; ни один HTTP-маршрут не создаёт учётную запись и не задаёт пароль.
