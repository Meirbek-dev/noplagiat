...
- **Публичный контур** - доступен на портале без авторизации и содержит только
  ...
  (release closure).
- **Внутренний контур** - детализация для авторизованных ролей: вход по логину и
  паролю (учётные записи создаёт CLI `manage-users`), ролевая модель RBAC с
  областью видимости по факультету и кафедре, журналирование каждого обращения,
  включая экспорт.
...
`vp run seed` заводит и фикстурные учётные записи. Свою собственную создайте тем
же CLI, которым это делается в продакшене - другого способа нет (ADR-017):

```bash
cargo run --manifest-path server/Cargo.toml --bin manage-users -- \
  create-user --username admin --role admin
```
...
```bash
cp deploy/env.example deploy/.env    # задайте пароли БД
...
- [docs/AUTH.md](docs/AUTH.md) - аутентификация и авторизация: CLI учётных записей, сеансы, роли, гарантии.
